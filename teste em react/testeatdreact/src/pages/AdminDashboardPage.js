...
import { auth, firebaseProjectId, firestorePersistence } from '../firebase';
import { exportCsv, exportTxt, exportXlsx } from '../services/exportService';
import { formatDateYYYYMMDD, startOfDay, endOfDay } from '../utils/date';
...
export default function AdminDashboardPage() {
  ...
  const [histFirst, setHistFirst] = useState(null);
  const [histLast, setHistLast] = useState(null);
    ...
    try {
      ...
      if (idQueryRaw) {
        ...
        if (Number.isFinite(maybeNumber) && maybeNumber > 0) {
          snap = await getDocs(query(feedbackCollection(), where('id', '==', maybeNumber), limit(10)));
        }
        if (!snap || snap.empty) {
          snap = await getDocs(query(feedbackCollection(), where('id', '==', idQueryRaw), limit(10)));
        }
      ...
      const start = startOfDay(new Date(histFrom));
      const end = endOfDay(new Date(histTo));

      const base = [where('timestamp', '>=', start), where('timestamp', '<=', end)];
      if (histGrau) base.push(where('grau_satisfacao', '==', histGrau));

      const pageSize = 50;
      let q1;
      if (direction === 'next' && histLast) {
        q1 = query(
          feedbackCollection(),
          ...base,
      ...
      } else if (direction === 'prev' && histFirst) {
        q1 = query(
          feedbackCollection(),
          ...base,
          orderBy('timestamp', 'desc'),
        ...
        q1 = query(feedbackCollection(), ...base, orderBy('timestamp', 'desc'), limit(pageSize));
      }
      ...
      const nextExists = direction === 'prev' ? true : snap.size === pageSize;
      setHistHasNext(nextExists);
  ...
  async function exportRange(type) {
    const start = startOfDay(new Date(exportFrom));
    const end = endOfDay(new Date(exportTo));
    ...
    const snap = await getDocs(query(feedbackCollection(), ...clauses));
    const rows = snap.docs.map((d) => ({ ...d.data(), docId: d.id }));
      ...
      <div className="dashboard-content">
        <section className="system-section">
          ...
          <div className="system-grid">
            ...
            <div className="system-card">
              <div className="k">Firebase</div>
              <div className="v">{firebaseProjectId || '—'}</div>
              <div className="s">
                Persistência: {firestorePersistence.enabled ? 'On' : 'Off'}
