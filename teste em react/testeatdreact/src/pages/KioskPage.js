    ...
    for (const item of q) {
      try {
        ...
        if (isPermissionDenied(err)) {
          ...
          setTimedMessage(
            'error',
            'Os registos pendentes não podem ser enviados: Firestore Rules a bloquear. Atualiza as rules no Firebase Console.',
            7000
          );
          break;
        }
  ...
  async function handleSubmit(grau) {
    ...
    try {
      if (soundOn) beep();
        ...
        if (!isOnline) {
          // `navigator.onLine` pode ser falso positivo (ex.: webviews). Tenta Firestore na mesma.
          try {
            ...
            if (isPermissionDenied(err)) {
              setTimedMessage(
                'error',
                'Não foi possível registar: Firestore Rules a bloquear (permission-denied). Atualiza as rules no Firebase Console.',
                7000
              );
              return;
            }
